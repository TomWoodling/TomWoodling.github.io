...
function getInputDirection() {
  var rawX = 0;
  var rawZ = 0;
  if (input.forward)  rawZ -= 1;  // W = toward -Z (into screen)
  if (input.backward) rawZ += 1;  // S = toward +Z (out of screen)
  if (input.left)     rawX -= 1;  // A = toward -X (left)
  if (input.right)    rawX += 1;  // D = toward +X (right)
  ...
  // Combine: forward/back along camera forward, left/right along camera right
  var dir = new THREE.Vector3(0, 0, 0);
  dir.addScaledVector(camForward, -rawZ);  // -rawZ because W=-1 should go along camForward
...
// --- Fox detection zone check (now based on orb proximity and detection bar) ---

function isInDogDetectionZone() {
  // Check if detection bar has any level (meaning orbs are detecting us)
  if (typeof detectionState !== 'undefined' && detectionState.level > 5) {
    return true;
  }
  // Also check proximity to active dogs
  if (typeof dogs !== 'undefined' && typeof dogsVisible !== 'undefined' && dogsVisible) {
    for (var i = 0; i < dogs.length; i++) {
      var dist = foxState.position.distanceTo(dogs[i].position);
      if (dist < C.DOG_DETECTION_FOR * 1.2) {
        return true;
      }
    }
  }
...
function updateFox(dt) {
  ...
  var sneaking = isInDogDetectionZone() && !sprinting;
  foxState.isSneaking = sneaking;
  foxState.nearDog = isInDogDetectionZone();
...
function foxRespawn() {
  ...
  // Reset detection and despawn dogs
  if (typeof resetDetection === 'function') resetDetection();
  if (typeof deactivateDogs === 'function') deactivateDogs();
}
