...
// ═══ MONSTER MANAGER ═══
// Cycles through MONSTER_ROSTER round-robin, independent of palettes/biomes.
var monsterManager = {
  rosterIdx:   0,    // which entry in MONSTER_ROSTER is next
  active:      false,
  cooldownTimer: 0,
  ...
  update: function(dt, carTransform, speed, boostActive) {
    ...
    if (this.active) {
      ...
      else if (current === 'godzilla') godzilla.update(dt, carTransform, speed);
      else if (current === 'crabs')    crabs.update(dt, carTransform, speed, boostActive);
    }
  },

  _activateNext: function(carTransform) {
    var key = MONSTER_ROSTER[this.rosterIdx % MONSTER_ROSTER.length];
    this.rosterIdx = (this.rosterIdx + 1) % MONSTER_ROSTER.length;
    this.active = true;
    ...
    dbg('Monster activated: ' + key, 'warn');
  },
...
function onMonsterDefeated(monsterKey) {
  monsterManager.active       = false;
  monsterManager.cooldownTimer = 0;
  dbg('Monster defeated! Next in ' + C.monsterCooldown + 's', 'warn');
}

// Legacy stub — no-op now that there are no biome transitions on the road.
function onBiomeChanged(key) {}
