...
var rFS = [
  ...
  'varying vec2 vUv;',
  'void main() {',
  '  float cl = smoothstep(0.008, 0.003, abs(vUv.x - 0.5)) * step(0.5, fract(vUv.y * 20.0 + scrollOffset));',
  '  float el = max(smoothstep(0.03, 0.02, vUv.x), smoothstep(0.97, 0.98, vUv.x));',
  '  float g = max(',
  '    smoothstep(0.005, 0.0, abs(fract(vUv.x * 12.0) - 0.5) - 0.48),',
  '    smoothstep(0.005, 0.0, abs(fract(vUv.y * 10.0 + scrollOffset) - 0.5) - 0.48)',
  '  ) * 0.08;',
...
var gFS = [
  ...
  'uniform float scrollOffset, time;',
  'varying vec3 vWP;',
  'void main() {',
  '  vec2 u = vWP.xz * 0.08;',
  '  u.y += scrollOffset * 0.08;',
  '  float g = max(',
